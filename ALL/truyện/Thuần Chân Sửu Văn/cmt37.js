...
addComment.addEventListener("click", function () {
    ...
    for (var i = 0; i < deleteIcons.length; i++) {
        deleteIcons[i].addEventListener("click", function () {
            ...
            localStorage.cmtData37 = JSON.stringify(commentsContainer.innerHTML);
        });
    }
...
document.body.onload = function (){
    if(!localStorage.cmtData37){
        localStorage.cmtData37 = null;
    }
    else
    {
        commentsContainer.innerHTML = JSON.parse(localStorage.cmtData37);
    }
    var deleteIcons = document.querySelectorAll(".container .comments div i");
    for (var i = 0; i < deleteIcons.length; i++) {
        deleteIcons[i].addEventListener("click", function () {
